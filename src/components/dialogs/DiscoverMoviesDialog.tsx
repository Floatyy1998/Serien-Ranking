...
const DiscoverMoviesDialog = ({ open, onClose }: DiscoverMoviesDialogProps) => {
  ...
  const handleSearch = async (page = 1) => {
    ...
    const response = await fetch(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.VITE_API_TMDB}&with_genres=${genreIds}&primary_release_year=${filters.year}&primary_release_date.gte=${releaseDateGte}&with_origin_country=${countryCodes}&page=${page}`
    );
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth='lg'>
      <DialogHeader title='Filme entdecken' onClose={handleClose} />
      <DialogContent>
