...
import { CheckCircle, Close, ChevronRight, PlaylistRemove } from '@mui/icons-material';
...
interface CompletedSeriesNotificationProps {
  series: Series[];
  onDismiss: () => void;
}

export const CompletedSeriesNotification: React.FC<CompletedSeriesNotificationProps> = ({
  series,
  onDismiss,
}) => {
  ...
  const markSeriesAsNotified = async (seriesId: number) => {
    ...
    try {
      const notificationRef = firebase.database().ref(
        `users/${user.uid}/completedSeriesNotifications/${seriesId}`
      );
  ...
  const handleDismissAll = async () => {
    if (user && series.length > 0) {
      ...
      series.forEach(s => {
        updates[`users/${user.uid}/completedSeriesNotifications/${s.id}`] = {
          dismissed: true,
          timestamp: Date.now(),
        };
      });
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          style={{
            background: `linear-gradient(135deg, ${currentTheme.status.success}20, ${currentTheme.background.default})`,
            borderColor: currentTheme.status.success + '40',
        ...
        >
          ...
          <div className="notification-content">
            <div className="notification-header">
              ...
              />
              <h3>Serie{series.length > 1 ? 'n' : ''} abgeschlossen</h3>
            </div>
            ...
            <div className="series-info">
              ...
              <div className="series-details">
                ...
                <p className="season-info">
                  ...
                  <span>
                    Komplett geschaut, keine neuen Folgen geplant
                  </span>
                </p>
              </div>
            ...
            {series.length > 1 && (
              <p className="counter">
                {currentIndex + 1} von {series.length} abgeschlossenen Serien
              </p>
            )}
          </div>
        </motion.div>
