import { Circle, Close, LiveTv, Star } from '@mui/icons-material';
...
export const UsersTab = React.memo<UsersTabProps>(({ data, theme }) => {
  ...
  // ─── User Deep-Dive Analysis ─────────────────────────────
  const userAnalysis = useMemo(() => {
    if (!selectedUser || userEvents.length === 0) return null;
    ...
    return {
      ...
      seriesAdded,
      seriesDeleted,
      hourActivity: hourActivity.map((count, hour) => ({ hour: `${hour}h`, count })),
      peakHour,
    };
      ...
      <AnimatePresence>
        {selectedUser && (
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {data.userProfiles[selectedUser]?.photoURL ? (
                  <img
                    src={data.userProfiles[selectedUser].photoURL}
                    alt=""
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 15, color: theme.text.primary }}>
                    {data.userProfiles[selectedUser]?.displayName ||
                      data.userProfiles[selectedUser]?.username ||
                      ...
                      <ResponsiveContainer width="100%" height={200}>
                        <PieChart>
                          <Pie
                            ...
                            labelLine={false}
                          >
                            {userAnalysis.genreData.map((_, i) => (
                              <Cell
                                key={i}
                    ...
                    <ResponsiveContainer width="100%" height={150}>
                      <BarChart data={userAnalysis.hourActivity}>
                        ...
                        <Bar dataKey="count" radius={[3, 3, 0, 0]} name="Events">
                          {userAnalysis.hourActivity.map((_, i) => (
                            <Cell
                              key={i}
                              fill={i === userAnalysis.peakHour ? '#ff6b6b' : theme.primary}
                              fillOpacity={i === userAnalysis.peakHour ? 1 : 0.6}
                            />
                          ))}
                        </Bar>
                      </BarChart>
