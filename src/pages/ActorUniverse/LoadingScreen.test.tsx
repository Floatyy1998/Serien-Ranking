...
describe('LoadingScreen', () => {
  it('renders the Actor Universe title and subtitle', () => {
    render(<LoadingScreen progress={0} />);
    expect(screen.getByText('Actor Universe')).toBeInTheDocument();
    expect(screen.getByText('Analysiere deine Serien...')).toBeInTheDocument();
  });

  it('renders the rounded progress percentage', () => {
    render(<LoadingScreen progress={49.6} />);
    expect(screen.getByText('50% - Sammle Schauspieler-Daten')).toBeInTheDocument();
  });

  it('shows 0% at the start', () => {
    render(<LoadingScreen progress={0} />);
    expect(screen.getByText('0% - Sammle Schauspieler-Daten')).toBeInTheDocument();
  });
});
