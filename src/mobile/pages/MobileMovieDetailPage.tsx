import { Delete, Info, People, Star, Visibility } from '@mui/icons-material';
...
export const MobileMovieDetailPage = memo(() => {
  ...
  const { user } = useAuth()!;
  const { movieList, addMovie, deleteMovie } = useMovieList();
  ...
  const handleDeleteMovie = useCallback(async () => {
    ...
    try {
      ...
      // Delete movie using context method
  ...
  return (
    <div>
          ...
          >
            ...
            {averageRating > 0 && (
              <span style={{ color: '#ffd700' }}>
                • ⭐ {averageRating} (
                {Object.keys(movie.rating || {}).filter((uid) => movie.rating![uid] > 0).length})
              </span>
            )}
          </div>
        ...
        <div style={{ padding: '20px', display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <motion.button
            ...
            style={{
              ...
              border: isWatched ? '1px solid rgba(255, 215, 0, 0.4)' : 'none',
              borderRadius: '12px',
              color: currentTheme.text.primary,
              fontSize: '16px',
              fontWeight: 600,
      ...
      {activeTab === 'cast' ? (
        <MobileCastCrew tmdbId={movie.id} mediaType="movie" seriesData={movie} />
      ) : (
        <div style={{ padding: '20px' }}>
          {/* Overview */}
