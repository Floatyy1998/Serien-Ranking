...
import { 
  Person,
  PersonAdd, Groups,
  Close, CheckCircle, Cancel, ArrowBack
...
export const MobileActivityPage: React.FC = () => {
  ...
  const getItemDetails = (activity: FriendActivity) => {
    ...
    if (activity.type === 'series_added' || activity.type === 'series_rated' || (activity as any).itemType === 'series') {
      ...
      return series;
    } else if (activity.type === 'movie_added' || activity.type === 'movie_rated' || (activity as any).itemType === 'movie') {
  ...
  const formatActivityMessage = (activity: FriendActivity): string => {
    ...
    } else if (activity.type === 'series_rated' || ((activity as any).itemType === 'series' && rating)) {
      return `hat "${itemTitle}" bewertet (${rating}/10)`;
    } else if (activity.type === 'movie_added') {
      return `hat "${itemTitle}" hinzugefügt`;
    } else if (activity.type === 'movie_rated' || ((activity as any).itemType === 'movie' && rating)) {
      return `hat "${itemTitle}" bewertet (${rating}/10)`;
    }
  ...
  useEffect(() => {
    ...
    const fetchMissingPosters = async () => {
      ...
      for (const activity of friendActivities) {
        ...
        if (itemType === 'series' || activity.type === 'series_added' || activity.type === 'series_rated') {
          const series = seriesList.find(s => s.id === tmdbId || s.id === Number(tmdbId));
          if (series && series.poster && typeof series.poster === 'object' && series.poster.poster) continue;
        } else if (itemType === 'movie' || activity.type === 'movie_added' || activity.type === 'movie_rated') {
          const movie = movieList.find(m => m.id === tmdbId || m.id === Number(tmdbId));
          if (movie && movie.poster && typeof movie.poster === 'object' && movie.poster.poster) continue;
        }
        
        // Check if activity already has posterPath
        if ((activity as any).posterPath || (activity as any).poster) continue;
        
        postersToFetch.push({ id: String(tmdbId), type: itemType === 'movie' ? 'movie' : 'series' });
      }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {sortedActivities.length === 0 ? (
              <div style={{
                ...
                <Groups style={{ fontSize: '48px', marginBottom: '16px', opacity: 0.3 }} />
              ...
              sortedActivities.map(activity => {
                ...
                const activityProfile = friendObj ? (friendProfiles[friendObj.uid] || friendObj) : null;
          ...
          <div>
            ...
            {friendRequests.length === 0 && sentRequests.length === 0 && (
              <div style={{
                ...
                <PersonAdd style={{ fontSize: '48px', marginBottom: '16px', opacity: 0.3 }} />
              ...
              <div style={{
                ...
                {searchResults.map((result) => (
                  <button
                    ...
                    disabled={result.isAlreadyFriend || result.hasPendingRequest || addingFriend}
                    style={{
                      width: '100%',
                      display: 'flex',
                      alignItems: 'center',
