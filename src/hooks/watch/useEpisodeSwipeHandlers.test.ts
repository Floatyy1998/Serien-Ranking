...
vi.mock('../../lib/watch/compactWatch', () => ({
...
vi.mock('../../lib/interaction/haptics', () => ({ hapticSuccess: mocks.hapticSuccess }));
...
vi.mock('../data/useWebWorkerTodayEpisodes', () => ({ useWebWorkerTodayEpisodes: () => [] }));
vi.mock('../rating/useQuickSeasonRating', () => ({
...
describe('useEpisodeSwipeHandlers – handleContinueEpisodeComplete', () => {
  it('marks the episode watched via the offline queue and fires an undo toast', async () => {
    ...
    expect(mocks.buildEpisodeWatchedUpdates).toHaveBeenCalledWith(
      ...
      expect.any(String),
      true,
      // Folgennummer und absolute Nummer: machen den Eintrag unabhaengig von der
      // Episoden-ID der Quelle und vom Staffelschnitt.
      expect.any(Number),
      undefined
    );
